"""
Phase 2: train English CTC on iSign with structural guidance.
...
def gloss_structural_loss(english_log_probs, gloss_log_probs, lengths, blank_id=0):
...
def motion_structural_target(features, lengths, landmark_dim=225, eps=1e-6):
    """
    Build q_t from pose velocity energy.

    features are expected to be [landmarks, velocity], shape (B, T, 450).
    Quantile normalization is per sample, so it works with raw or normalized
    features and is robust to signer/video scale.
    """
    velocity = features[:, :, landmark_dim:]
    motion = velocity.abs().mean(dim=-1)
    q = torch.zeros_like(motion)
    for b in range(features.size(0)):
        length = int(lengths[b].item())
        if length <= 1:
            continue
        sample_motion = motion[b, :length]
        lo = torch.quantile(sample_motion, 0.10)
        hi = torch.quantile(sample_motion, 0.90)
        q[b, :length] = ((sample_motion - lo) / (hi - lo + eps)).clamp(0.0, 1.0)
    return q


def motion_structural_loss(english_log_probs, features, lengths, blank_id=0):
    r = 1.0 - english_log_probs.exp()[..., blank_id]
    q = motion_structural_target(features, lengths).detach()
    mask = torch.arange(r.size(1), device=r.device)[None, :] < lengths[:, None]
    return F.binary_cross_entropy(r[mask], q[mask])


def compute_structural_loss(model, english_lp, features, encoded, feat_lens, struct_source):
    if struct_source == "none":
        return english_lp.new_zeros(())
    if struct_source == "motion":
        return motion_structural_loss(english_lp, features, feat_lens, model.blank_id)
    if struct_source == "gloss":
        gloss_lp = model.gloss_log_probs(features, encoded)
        return gloss_structural_loss(english_lp, gloss_lp, feat_lens, model.blank_id)
    raise ValueError(f"Unknown struct_source: {struct_source}")
...
def train_epoch(model, loader, criterion, optimizer, device, lam, struct_source):
    model.train()
    ...
    for batch in loader:
        ...
        struct = compute_structural_loss(
            model, english_lp, features, encoded, feat_lens, struct_source
        )
        loss = ctc + lam * struct
...
@torch.no_grad()
def validate(model, loader, criterion, vocab, device, lam, struct_source):
    model.eval()
    totals = {"loss": 0.0, "ctc": 0.0, "struct": 0.0, "items": 0}
    refs, hyps = [], []
    for batch in loader:
...
def main():
    ...
    parser.add_argument("--gloss_checkpoint", default=None)
    parser.add_argument("--gloss_norm_stats", default=None)
    ...
    parser.add_argument("--struct_source", choices=["motion", "gloss", "none"], default="motion")
    parser.add_argument("--lambda_struct", type=float, default=0.05)
    parser.add_argument("--input_dim", type=int, default=450)
    ...
    ckpt = None
    if args.gloss_checkpoint is not None:
        ckpt = torch.load(args.gloss_checkpoint, map_location="cpu")
    if args.struct_source == "gloss" and ckpt is None:
        raise ValueError("--struct_source gloss requires --gloss_checkpoint")

    stats_path = args.norm_stats or args.gloss_norm_stats
    ...
    model = TwoHeadCTCModel(
        input_dim=ckpt["input_dim"] if ckpt is not None else args.input_dim,
        hidden_size=ckpt["hidden_size"] if ckpt is not None else args.hidden_size,
        num_layers=ckpt["num_layers"] if ckpt is not None else args.num_layers,
        dropout=ckpt["dropout"] if ckpt is not None else args.dropout,
        gloss_vocab_size=ckpt["gloss_vocab_size"] if ckpt is not None else 2,
        english_vocab_size=vocab.size,
        blank_id=ckpt["blank_id"] if ckpt is not None else 0,
    ).to(device)
    if ckpt is not None:
        model.encoder.load_state_dict(ckpt["encoder"])
        model.gloss_head.load_state_dict(ckpt["gloss_head"])
    elif args.head_epochs > 0:
        print("No pretrained checkpoint supplied; skipping head-only warmup.")
        args.head_epochs = 0
